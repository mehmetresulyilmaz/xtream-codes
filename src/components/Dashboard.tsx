  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden relative font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-24 md:w-80 border-r border-slate-200 flex flex-col bg-white/60 backdrop-blur-3xl transition-all duration-500 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-4 border-t border-slate-100">
           <div className="hidden md:block mb-4 p-5 bg-slate-50/80 rounded-2xl border border-slate-200/50 backdrop-blur-sm">
              ...
              <div className="flex flex-col gap-2 mt-3 pl-0.5">
                 <div className="flex items-center gap-2">
                    <div className={`h-1.5 w-1.5 rounded-full ${authData.user_info.status === 'Active' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' : 'bg-red-500'}`} />
                 ...
                 <div className="text-[10px] text-slate-400 font-bold flex items-center gap-2 bg-slate-100/50 w-fit px-2 py-0.5 rounded-md border border-slate-200/30">
                    <span className="opacity-60">BİTİŞ:</span> 
                    <span className="text-slate-600">
                      {authData.user_info.exp_date && authData.user_info.exp_date !== "null" 
                        ? new Date(parseInt(authData.user_info.exp_date) * 1000).toLocaleDateString('tr-TR')
                        : 'Süresiz'}
                    </span>
                 </div>
              </div>
      ...
      <Dialog open={showPinSetup} onOpenChange={setShowPinSetup}>
        <DialogContent className="bg-white border-slate-200 text-slate-800 w-[90vw] sm:max-w-md rounded-2xl sm:rounded-[2.5rem] shadow-2xl p-6 sm:p-10">
          <DialogHeader>
            <DialogTitle className="text-xl sm:text-2xl font-black italic text-slate-900">Ebeveyn PIN Kodu Oluştur</DialogTitle>
          ...
          <div className="py-4 sm:py-6">
            ...
            <Input
              ...
              className="bg-slate-100 border-slate-200 text-slate-900 text-2xl tracking-[1em] text-center h-14 sm:h-16 rounded-xl sm:rounded-2xl shadow-inner focus:ring-orange-500/10"
              placeholder="0000"
            />
          </div>
          <DialogFooter>
            <Button onClick={handleSavePin} className="w-full bg-slate-900 hover:bg-slate-800 text-white h-12 sm:h-14 rounded-xl sm:rounded-2xl font-black text-sm sm:text-base shadow-xl">PIN Kodunu Kaydet</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showPinVerify} onOpenChange={setShowPinVerify}>
        <DialogContent className="bg-white border-slate-200 text-slate-800 w-[90vw] sm:max-w-md rounded-2xl sm:rounded-[2.5rem] shadow-2xl p-6 sm:p-10">
          <DialogHeader>
            <DialogTitle className="text-xl sm:text-2xl font-black italic text-slate-900">PIN Kodunu Girin</DialogTitle>
            <DialogDescription className="text-slate-500 font-medium text-xs sm:text-sm">
          ...
          <DialogFooter>
            <Button onClick={handleVerifyPin} className="w-full bg-orange-600 hover:bg-orange-700 text-white h-12 sm:h-14 rounded-xl sm:rounded-2xl font-black text-sm sm:text-base shadow-xl shadow-orange-600/20">Kilidi Aç</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={!!selectedSeries} onOpenChange={() => setSelectedSeries(null)}>
        <DialogContent className="bg-white border-slate-100 text-slate-800 w-[95vw] sm:max-w-5xl h-[90vh] sm:h-[85vh] p-0 overflow-hidden flex flex-col shadow-2xl rounded-[1.5rem] sm:rounded-[3rem]">
           {selectedSeries && (
             <>
               <div className="relative h-64 sm:h-80 flex-shrink-0">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-12 flex gap-6 sm:gap-10 items-end">
                     <div className="h-44 sm:h-64 w-32 sm:w-44 rounded-2xl sm:rounded-[2rem] overflow-hidden shadow-2xl border border-white/10 transform -translate-y-4 hidden sm:block">
                        <img src={selectedSeries.series.cover} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                     </div>
                     <div className="flex-1 flex flex-col justify-end">
                        <div className="flex items-center gap-3 mb-3 sm:mb-4">
                           <div className="px-2.5 py-0.5 sm:px-3 sm:py-1 bg-orange-600 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-orange-600/20">
                              {selectedSeries.info?.info?.genre?.split(',')?.[0] || 'Series'}
                           </div>
                           <div className="px-2.5 py-0.5 sm:px-3 sm:py-1 bg-white/10 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-300 border border-white/10 backdrop-blur-md">
                              {selectedSeries.info?.info?.releaseDate || '2024'}
                           </div>
                        </div>
                        <h2 className="text-3xl sm:text-5xl font-black tracking-tighter leading-[0.9] mb-3 sm:mb-4 text-white drop-shadow-2xl">{selectedSeries.series.name}</h2>
                        <div className="flex items-center gap-4 sm:gap-6 text-[10px] sm:text-xs font-black text-zinc-400 uppercase tracking-[0.2em] mt-1 sm:mt-2">
                           <span className="flex items-center gap-2 text-orange-500">
                             <Library className="h-3 w-3 sm:h-4 sm:w-4" /> {selectedSeries.series.rating || '8.4'}
                           </span>
                           <span className="opacity-20">|</span>
                           <span>{Object.keys(selectedSeries.info?.episodes || {}).length} Sezon</span>
                           <span className="hidden sm:inline opacity-20">|</span>
                           <span className="hidden sm:inline text-zinc-500">{selectedSeries.info?.info?.cast?.split(',')?.[0]}</span>
                        </div>
                     </div>
                  </div>
               </div>

               <div className="flex-1 overflow-hidden flex flex-col p-6 sm:p-12 pt-4 sm:pt-6">
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 sm:pr-6">
                     <div className="space-y-8 sm:space-y-12">
                        {Object.entries(selectedSeries.info?.episodes || {}).map(([seasonNum, episodes]: [string, any]) => (
                           <div key={seasonNum}>
                              <div className="flex items-center gap-4 mb-6 sm:mb-8">
                                 <h3 className="text-[10px] sm:text-sm font-black text-orange-500 uppercase tracking-[0.3em] sm:tracking-[0.5em]">SEZON {seasonNum}</h3>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                                 {episodes.map((ep: any) => (
                                    <button 
                                      ...
                                      className="flex items-center gap-4 sm:gap-5 p-3 sm:p-4 rounded-2xl sm:rounded-[1.5rem] bg-white/5 border border-white/5 hover:bg-white/10 hover:border-orange-500/30 transition-all group text-left relative overflow-hidden"
                                    >
                                       <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl bg-orange-600/10 flex items-center justify-center flex-shrink-0 group-hover:bg-orange-600 transition-all shadow-inner">
                                          <Play className="h-4 w-4 sm:h-5 sm:w-5 fill-current text-orange-500 group-hover:text-white" />
                                       </div>
                                       <div className="flex-1 min-w-0">
                                          <p className="font-black truncate group-hover:text-orange-500 transition-colors uppercase tracking-tight text-[11px] sm:text-sm">Bölüm {ep.episode_num || ep.id}: {ep.title}</p>
                                          <p className="text-[9px] sm:text-[10px] text-zinc-500 group-hover:text-zinc-300 font-bold truncate mt-0.5 sm:mt-1 uppercase tracking-wider">
                                            {ep.info?.duration || '45 dk'} • {ep.info?.plot?.slice(0, 40) || 'Bölüm detayları bulunmamaktadır.'}...
                                          </p>
                                       </div>
                                    </button>
                                 ))}
